...
const columnas = {
  clientes: [
    ...
    { title: "Acciones", key: "acciones", render: (_, record, i) => <AccionesTabla record={record} maestro="clientes" /> },
  ],
  proveedores: [
    { title: "Nombre", dataIndex: "nombre" },
    { title: "Teléfono", dataIndex: "telefono" },
    { title: "Email", dataIndex: "email" },
    { title: "Acciones", key: "acciones", render: (_, record, i) => <AccionesTabla record={record} maestro="proveedores" /> },
  ],
  productos: [
    { title: "Nombre", dataIndex: "nombre" },
    { title: "Categoría", dataIndex: "categoria" },
    { title: "Acciones", key: "acciones", render: (_, record, i) => <AccionesTabla record={record} maestro="productos" /> },
  ],
  materias: [
    { title: "Nombre", dataIndex: "nombre" },
    { title: "Stock", dataIndex: "stock" },
    { title: "Acciones", key: "acciones", render: (_, record, i) => <AccionesTabla record={record} maestro="materias" /> },
  ],
  recetas: [
    { title: "Nombre", dataIndex: "nombre" },
    { title: "Acciones", key: "acciones", render: (_, record, i) => <AccionesTabla record={record} maestro="recetas" /> },
  ],
  empleados: [
    { title: "Nombre", dataIndex: "nombre" },
    { title: "Puesto", dataIndex: "puesto" },
    { title: "Acciones", key: "acciones", render: (_, record, i) => <AccionesTabla record={record} maestro="empleados" /> },
  ],
  direcciones: [
    { title: "Nombre", dataIndex: "nombre" },
    { title: "Dirección", dataIndex: "direccion" },
    { title: "Acciones", key: "acciones", render: (_, record, i) => <AccionesTabla record={record} maestro="direcciones" /> },
  ],
  unidades: [
    { title: "Nombre", dataIndex: "nombre" },
    { title: "Símbolo", dataIndex: "simbolo" },
    { title: "Acciones", key: "acciones", render: (_, record, i) => <AccionesTabla record={record} maestro="unidades" /> },
  ],
};

function AccionesTabla({ record, maestro }: { record: any; maestro: string }) {
  return (
    <Space>
...
export default function Maestros() {
  const [maestro, setMaestro] = useState("clientes");
  const [busqueda, setBusqueda] = useState("");
  const [modalOpen, setModalOpen] = useState(false);
  const [form] = Form.useForm();

  const data = dataEjemplo[maestro].filter((item) =>
    ...
    <Layout style={{ minHeight: "80vh", background: "#f4f6fa" }}>
      <Sider width={220} style={{ background: "#fff", borderRadius: 12, margin: 16, boxShadow: "0 2px 8px #e6e6e6" }}>
        <Menu
          ...
          items={maestros.map((m) => ({ ...m, label: <span style={{ fontSize: 16 }}>{m.icon} {m.label}</span> }))}
          onClick={({ key }) => setMaestro(key)}
        />
      </Sider>
